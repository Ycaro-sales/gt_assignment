...
std::string print_distances(std::vector<distance> distances){
    ...
    for(int i = 0; i < distances.size(); i++){
        dists_str += std::to_string(i+1) + ":" + std::to_string(distances[i]) + " ";
    }
...
{
    source -= 1;
    std::priority_queue<source_distance, std::vector<source_distance>, std::greater<source_distance>> Q;
    ...
    Q.push(source_distance(dists[source], source));
